...
function find_address(hash, cb) {
  ...
  Address.findOne({a_id: hash}, {txs: 0}, function(err, address) {
    if(address) {
      return cb(address);
    } else {
      return cb();
  ...
  find_address(hash, function(address) {
    if (address) {
      ...
      if ( hash == 'coinbase' ) {
        ...
        var unique = true;
		var index = null;
		//Address.findOne({a_id: hash, "txs.addresses": txid}, function(err, txExists){
	//	Address.aggregate([
    //
 //   {$match: {"a_id": hash, "txs.addresses": txid}},
  //  {$project: {
    //    txs: {$filter: {
    //        input: '$txs',
    //        as: 'txs',
   //         cond: {$eq: ['$$txs.addresses', txid]}
   //     }},
    //    _id: 0
  //  }}
  Address.find(
{"a_id": hash,"txs.addresses": txid}, 
{txs: {$elemMatch: {addresses: txid}}}
,function(err, txExists){
		if (txExists){
			unique = false;
		}
		});
         // var tx_array = [];
          ...
          if (unique == true) {
           ...
           //   tx_array.shift();
			...
			Address.findByIdAndUpdate(address._id, {$push: {"txs": {"addresses": txid, "type": type}}}, {upsert: true}, function() {
              return cb();
            });
	
            Address.update({a_id:hash}, {$set:{
      ...
      lib.get_blockhash(start + x, function(blockhash){
        if (blockhash) {
          lib.get_block(blockhash, function(block) {
            if (block) {
		    if (x % 50 === 0) {
			    console.log('at block %s', block.height);
		    }
